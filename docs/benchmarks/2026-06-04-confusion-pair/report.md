...
- The **dominant** within-session confusion is **→ 松浦**, the session's dominant speaker (most/longest turns), which acted as a stickiness attractor: **28 chunks were wrongly absorbed by 松浦 vs 2 by 神野** (14×). The main victim was 上東 (21 of its chunks mislabeled), heavily in the 2026-04-21 session.
- **Static pairwise similarity did not predict live behavior.** The biggest live attractor (松浦) is not 神野's nearest neighbour, and a low-similarity pair (松浦↔森谷, 0.490) produced 7 live confusions. Live assignment is driven by per-chunk audio embeddings + diarization windowing + Viterbi stay-bias, not by stored-centroid proximity alone.

> **Correction (2026-06-10, post-merge):** §4 originally attributed 松浦's attractor effect to a `hitCount` tie-breaker / high embedding norm. That mechanism is **wrong for Manual mode** — `loadProfiles` resets every profile's `hitCount` to 1 and `suppressLearning=true` freezes it, so the tie-breaker is inert; and cosine is scale-invariant, so norm cannot affect ranking. The empirical counts below are unaffected; only the *mechanism* in §4/§5 is corrected. The leading mechanism is rolling-window granularity (short turns swallowed by the dominant speaker's 15 s window) compounded by Viterbi stay-bias — to be confirmed by the raw-cosine diagnostic in the 2026-06-10 handoff.
- **Critical caveat:** `speakers.json` is the 2026-06-04 snapshot; the sessions are from 2026-04-21/23. ~6 weeks of additional post-hoc learning have refined the profiles since. The user's *lived* 神野 problem occurred with **earlier, less-separated** profile states this replay cannot reproduce — and **no April backup of `speakers.json` exists (confirmed 2026-06-10)**, so that comparison is permanently closed. The actionable, reproducible problem is therefore the current-profile dominant-speaker stickiness (上東/今村 → 松浦).
...
1. **The attractor is the session's dominant speaker (松浦), not a well-trained or high-norm profile.** In Manual mode `hitCount` is reset to 1 for every loaded profile and frozen (`suppressLearning`), so the `hitCount` tie-breaker is inert; and cosine is scale-invariant, so 松浦's high norm is irrelevant to ranking. 松浦 wins because it *dominates the audio* (longest/most turns), which makes it the persistent `lastConfirmedId` and the Viterbi-favored state, and fills most rolling windows. 松浦 absorbed 28 chunks; 神野 absorbed 2.
2. **Diarization windowing is the leading mechanism.** A 15 s rolling window with 7 s accumulation means a short turn by speaker B inside speaker A's monologue is embedded in an A-dominated window and inherits A's label (`findRelevantSegment` picks the max-overlap segment). 上東's short agenda interjections inside 松浦's long 04-21 monologue are the clearest case (13 → 松浦). *Whether the per-chunk audio is genuinely 松浦-dominated (windowing) or 上東's stored centroid is simply under-representative needs the raw-cosine diagnostic (see handoff).*
3. **Viterbi stay-bias (0.9)** compounds it: once 松浦 is confirmed, brief other-speaker chunks without preceding silence don't trigger `resetForSpeakerChange()` and tend not to flip.
...
5. **Session dependence:** 上東 was 35 % correct in 04-21 (heavy 松浦 monologue) but 88 % in 04-23 (more balanced turns). Confusion is a property of session dynamics (overlap, turn length, who dominates each window), not a fixed property of a profile pair — consistent with the dominant-speaker stickiness mechanism.

**Bottom line:** confusion is dominated by **stickiness toward the session's dominant talker** (windowing + Viterbi stay-bias), not by 神野's embedding overlap and not by profile training — at least with the current profile snapshot.
...
| **Runtime margin penalty / anti-stickiness** | **Most promising, but re-targeted at dominant-speaker stickiness, not the silent profile.** First run the raw-cosine diagnostic to learn whether misattributed chunks are window-swallowed (raw diarizer already says 松浦) or smoother-flipped (raw says 上東, Viterbi/tie-break flips to 松浦). Then the fix follows: if window-swallowed → finer diarization granularity / sub-window attribution so short interjections aren't absorbed; if smoother-flipped → a similarity-margin requirement before staying on the confirmed speaker, or a shorter `resetForSpeakerChange` trigger. **Do NOT touch the `hitCount` tie-breaker — it is inert in Manual mode.** Target metric: 上東/今村 → 松浦 rate. |

**Single highest-value follow-up (NOW BLOCKED):** re-running Part B against a session-time `speakers.json` snapshot would isolate whether the lived 神野 problem was an earlier-profile-state property. **No April backup exists (confirmed 2026-06-10), so this path is closed.** Treat the current finding as the actionable baseline: the reproducible, current-profile problem is dominant-speaker stickiness (上東/今村 → 松浦), and that is where the next engineering effort should go.
