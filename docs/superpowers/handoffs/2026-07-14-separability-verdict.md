...
**後日談 (2026-07-17): PR #90/#91 とも squash マージ済み・後片付け済み。
Release v2.4.91 公開済み**（#90+#91 の集約、機能変更なし＝アプリは v2.4.89 と同一、
app zip 添付）。テストゲートは 803 件 + ベンチ内ユニット 17 件で失敗ゼロ。
未着手の残作業はなく、次セッションは「次の一手」の 1（loopback brainstorming）から
クリーンに開始できる。
...
> PR #90/#91 はマージ・リリース（v2.4.91）・後片付けまで完了済みです。
> システムオーディオ直接キャプチャ（loopback）による Zoom 遠端音声のデジタル取得を
> superpowers:brainstorming から設計してください。実会議の loopback 同時録音が
> あれば先にそれを確認（あれば実装前に separability プロトコルで効果を事前計測）。」
